	...
	factory: (require) => {
		...
		*/
		...
		function MainPanel({ usePanelInfo, renderSlot }) {
			const key = usePanelInfo((info) => info.activePanelId) ?? "conversation";
            if (key !== "conversation") return renderSlot("main", {}, { entryKey: key });
            return (0, react_jsx_runtime.jsxs)("div", {
                style: { display: "flex", flexDirection: "column", height: "100%", minHeight: 0, minWidth: 0 },
                children: [renderSlot("sm75.statusbar", {}),
                    (0, react_jsx_runtime.jsx)("div", { style: { flex: 1, minHeight: 0, minWidth: 0 }, children: renderSlot("main", {}, { entryKey: key }) })]
            });
		}
		/**
		...
		var ThemePresenter = class {
			...
			*/
			apply(snapshot) {
				...
				document.documentElement.style.setProperty("--dsh-font-scale", String(snapshot.fontSize / 14));
				document.documentElement.style.setProperty("--ui-fs", String(snapshot.fontSize / 14));
			...
			dispose() {
				...
				document.documentElement.style.removeProperty("--dsh-font-scale");
				document.documentElement.style.removeProperty("--ui-fs");
				const body = document.body;
			...
			ctx.effect(() => {
				...
				const disposeRegistration = ctx.slots.register({
					...
					children: {
						...
						"sm75.statusbar": { kind: "single", scope: "root" },
						"shell.overlay": {
							kind: "list",
							scope: "root"
						}
