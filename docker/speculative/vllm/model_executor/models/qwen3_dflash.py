...
class DFlashQwen3Model(nn.Module):
    ...
    def _normalize_context_k(self, all_k: torch.Tensor) -> torch.Tensor:
        ...
        if verified is None:
            layers, head_dim = self._k_norm_weights.shape
            probe = torch.ones(
                (layers, 1, 1, head_dim), dtype=all_k.dtype, device=all_k.device
            )
            grouped_probe = torch.empty_like(probe)
            ops.rms_norm(
                grouped_probe, probe, self._k_norm_weights, self._rms_norm_eps
            )
            verified = torch.equal(
                grouped_probe, self._normalize_context_k_per_layer(probe)
            )
            self._batched_k_norm_runtime_verified = verified
        if not verified:
            return self._normalize_context_k_per_layer(all_k)
        output = torch.empty_like(all_k)
        ops.rms_norm(output, all_k, self._k_norm_weights, self._rms_norm_eps)
        return output
...
class DFlashQwen3ForCausalLM(Qwen3ForCausalLM):
    ...
    def combine_hidden_states(
        ...
        # AWQ auxiliary states may be FP32; the draft FC transport is FP16.
