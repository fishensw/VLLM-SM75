...
def _use_sm75_bf16_emulation() -> bool:
...
class DFlash2Qwen3DecoderLayer(DFlashQwen3DecoderLayer):
    def __init__(
        ...
        *,
        ...
        self.use_sm75_bf16_emulation = _use_sm75_bf16_emulation()
        if self.use_sm75_bf16_emulation:
            runtime_dtype = vllm_config.model_config.dtype
            if runtime_dtype != torch.float16:
                raise ValueError(
                    "BF16 DFlash2 emulation on SM75 requires FP16 transport; "
...
class DFlash2Qwen3ForCausalLM(DFlashQwen3ForCausalLM):
    ...
    def compute_candidates(
        ...
        if not isinstance(
            ...
            raise ValueError(
                "DFlash2 requires an unquantized draft LM head for candidate TopK; "
        ...
        if values.shape[-1] > top_k:
            values, selected = torch.topk(values, top_k, dim=-1)
