...
def _dflash_warmup_batch_size(
    max_num_reqs: int, max_num_tokens: int, query_len: int, *, profile_only: bool
) -> int:
    """Keep serving-only warmup within the allocated token buffers."""
    if profile_only or query_len < 1:
        return 0
    return max(0, min(max_num_reqs, max_num_tokens // query_len))
...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    def load_model(self, load_dummy_weights: bool = False, *args, **kwargs) -> None:
        ...
        with DeviceMemoryProfiler() as m:
            ...
            if isinstance(self.speculator, DraftModelSpeculator):
                # SM75 FP8 repacking leaves large inactive allocator blocks.
                # Release only unused cache before allocating draft weights.
                # This runs once during loading, never during forward/Graph.
                if (
                    ...
                    logger.info(
                        "SM75 FP8 DFlash loader cache release: reserved %.3f -> %.3f GiB; allocated %.3f GiB",
    ...
    def capture_model(self, *, profile_only: bool = False) -> int:
        ...
        nreq = _dflash_warmup_batch_size(
            self.max_num_reqs, self.max_num_tokens, self.decode_query_len,
            profile_only=profile_only,
        )
        if nreq and isinstance(self.speculator, DFlashSpeculator):
...
# Local SM75 native FA2 Graph adapter; feature-gated.
import sm75_fa2_graph as _sm75_fa2_graph
_sm75_fa2_graph.install()

# Local SM75 B1 speculative GDN metadata staging.
import sm75_gdn_metadata as _sm75_gdn_meta
_sm75_gdn_meta.install()
