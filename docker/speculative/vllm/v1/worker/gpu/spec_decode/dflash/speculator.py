...
def prepare_dflash_inputs(
    ...
    if warmup_all_blocks:
        # The input preparation runs outside CUDA graphs. Compile every block
        # specialization reachable by prefill/decode before serving requests.
        # warmup compiles/loads only; it does not launch on dummy buffers.
        block = min(256, triton.next_power_of_2(1 + num_query_per_req))
        while block <= 256:
            _prepare_dflash_inputs_kernel.warmup(
                *kernel_args,
                **{**kernel_kwargs, "BLOCK_SIZE": block},
                grid=(num_reqs, triton.cdiv(max_tokens_per_req, block)),
            )
            block *= 2
