...
docker run -d --name vllm-sm75-next-ultra-0924 \
  --gpus all --restart no \
  --shm-size 64g --memory 112g --memory-swap 112g \
...
镜像环境：`NCCL_P2P_LEVEL=SYS`、`OMP_NUM_THREADS=1`、`VLLM_SM75_QWEN38_HC_GEMV=1`、`VLLM_FLASHINFER_WORKSPACE_BUFFER_SIZE=134217728`。
