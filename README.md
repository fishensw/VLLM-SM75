...
启用 firefly prefill：设 `VLLM_FIREFLY=1`（默认关闭，对 int4 权重 + fp16/bf16 激活、W8A8-FP8 大 M prefill 生效）。`docker/run-v0.1.2.sh` 暂未透传该变量，需手动 `docker run --env VLLM_FIREFLY=1 …` 或在脚本 `docker run` 段追加 `--env VLLM_FIREFLY=1`。
