...
class BuildContract(unittest.TestCase):
    def test_official_prebuilt_base(self):
        text = (ROOT / 'docker/Dockerfile.vllm-sm75-v0.1.2').read_text(encoding='utf-8')
        self.assertIn('vllm/vllm-openai:v0.28.0-cu129@sha256:', text)
        script = (ROOT / 'docker/build-v0.1.2.sh').read_text(encoding='utf-8')
    ...
    def test_unified_final_target(self):
        ...
        launch = (ROOT / 'docker/run-v0.1.2.sh').read_text(encoding='utf-8')
        self.assertIn('image=vllm-sm75:v0.1.2', launch)
