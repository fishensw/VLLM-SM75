...
class HummingFP8ScaledMMLinearKernel(FP8ScaledMMLinearKernel):
    ...
    def can_implement(
        ...
        scale_group = config.weight_quant_key.scale.group_shape
        if (
            current_platform.is_cuda()
            and current_platform.is_device_capability(75)
            and config.input_dtype == torch.float16
            and config.out_dtype == torch.float16
            and (scale_group.is_per_tensor() or scale_group.is_per_channel())
        ):
            # Humming 0.1.12 casts the FP32 accumulator to FP16 before its
            # channel-scale epilogue. Large unscaled FP8 sums can overflow
            # even when the final scaled result is finite. CT selects the
            # kernel while its key is still tensorwise, then expands scales
            # to channels, so both keys must be excluded. Its activation key
            # remains FP8 metadata even on W8A16; use the actual input dtype.
            return False, (
                "Humming FP8 with FP16 input/output and tensor/channel scales "
                "can overflow before scaling on SM75; use the Marlin backend"
            )
        return True, None
