...
"""SM75 overlay: MarlinFP8ScaledMMLinearKernel(fp8 weight-only Marlin)。

全文件拷贝自上游 v0.29.0, 与上游行为一致(fp8 恒走 weight-only Marlin)。
firefly-fp8 prefill(大 M int8 IMMA)已移除(sm75 实测 firefly-fp8 不比 marlin 快,
fused 慢 ~1.7x; fp8 加速走 VLLM_FIREFLY_AR), 见
vllm/model_executor/layers/quantization/utils/firefly.py 的 firefly_active_fp8。
"""
...
class MarlinFP8ScaledMMLinearKernel(FP8ScaledMMLinearKernel):
    ...
    def apply_weights(
        ...
        # fp8 恒走上游 weight-only Marlin(firefly-fp8 prefill 已移除, 见文件头)。
        if self.block_quant:
