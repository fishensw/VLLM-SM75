...
_PREBUILT_EXTENSION = "vllm.third_party.flash_qla_sm75.flash_qla_sm75_gdn"
_PREBUILT_EXTENSION_PATH_ENV = "FLASH_QLA_SM75_PREBUILT_EXTENSION_PATH"
...
def _load_ext():
    ...
    if _EXT_LOAD_ERROR is not None:
        raise RuntimeError(
            "SM75 FlashQLA extension initialization previously failed. "
            "The original loader or compiler error is chained below."
        ) from _EXT_LOAD_ERROR
    if not torch.cuda.is_available():
        raise RuntimeError("SM75 FlashQLA backend requires CUDA.")
    ...
    try:
        ...
        if exc.name != _PREBUILT_EXTENSION:
            _EXT_LOAD_ERROR = exc
            raise RuntimeError(
                "Bundled SM75 FlashQLA extension has a missing dependency."
            ) from exc
    except Exception as exc:
        _EXT_LOAD_ERROR = exc
        raise RuntimeError(
            "Bundled SM75 FlashQLA extension failed to load. Rebuild the image "
            "built for the active CUDA and PyTorch versions."
        ) from exc

    if os.environ.get(_ALLOW_JIT_ENV, "0") != "1":
        error = RuntimeError(
            "Bundled SM75 FlashQLA extension is missing and runtime JIT is "
            f"disabled. Set {_PREBUILT_EXTENSION_PATH_ENV} to the image-owned "
            f"extension or set {_ALLOW_JIT_ENV}=1 only for development."
        )
        _EXT_LOAD_ERROR = error
        raise error
    ...
    try:
        _EXT = load(
            ...
            sources=[str(src)],
            extra_cuda_cflags=["-O3", *_SM75_GENCODE_FLAGS],
            extra_cflags=["-O3"],
            verbose=bool(int(os.environ.get("FLASH_QLA_SM75_VERBOSE_BUILD", "0"))),
        )
    except Exception as exc:
